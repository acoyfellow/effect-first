...
const prefixLinks = (html: string) => html.replace(/href="\//g, 'href="/old/')

const renderIndex = (text: string): string => {
  ...
  while (i < lines.length) {
    ...
    if (routeMatch) {
      ...
      while (i < lines.length && (lines[i] ?? "").match(/^\s{10,}/)) {
        ...
        } else if (!trimmed.startsWith("Token counts") && trimmed.length > 0) {
          fullDesc += " " + trimmed
        }
        i++
      }
  ...
  const tableRows = routes
    .map((route) => {
      const link = route.path.startsWith("/bundle")
        ? `<code>${esc(route.path)}</code>`
        : `<a href="${esc(route.path)}" class="route-link"><code>${esc(route.path)}</code></a>`
      return `<tr><td>${link}</td><td class="tokens">${esc(route.tokens)}</td><td>${esc(route.desc)}${route.use ? `<br><em style="color:#8b949e">${esc(route.use)}</em>` : ""}</td></tr>`
    })
    .join("\n")
...
<nav>${routes
    .filter((route) => !route.path.startsWith("/bundle") && route.path !== "/health")
    .map((route) => `<a href="${esc(route.path)}">${esc(route.path)}</a>`)
    .join("")}</nav>
<table>
...
const renderContent = (text: string): string => {
  ...
  return `<nav><a href="/">← index</a></nav>\n${html}`
}

export const toLegacyHtml = (text: string, route: string): string => {
  const isIndex = route === "/"
  const body = isIndex ? renderIndex(text) : renderContent(text)
  return prefixLinks(
    `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>effect-first${route !== "/" ? " — " + route : ""}</title><style>${CSS}</style></head><body>${body}</body></html>`
  )
}
